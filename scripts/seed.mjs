...
const menuItems = [
  // 본도시락 (1)
  ...["본격도시락(카츠, 전)", "본격도시락(스팸두부조림)", "바싹불고기 반상", "숯불닭구이 반상", "깻잎제육 반상", "버섯소불고기 반상", "꽈리닭구이 반상", "궁중떡갈비 반상", "스크램블드 에그 치킨마요", "스크램블드 에그 치킨마요(곱배기)", "울릉도 나물 버섯영양밥", "저녁식사 안해용"].map((n, i) => ({ restaurantId: 1, name: n, itemType: "main", sortOrder: i + 1 })),

  // 본 비빔밥(본죽&비빔밥) (2)
  ...["6가지 나물 비빔밥", "제육볶음 나물 비빔밥", "소불고기 나물 비빔밥", "낙지 김치 비빔밥", "참치야채 비빔밥", "삼계죽", "쇠고기야채죽", "참치야채죽", "7가지 야채죽", "저녁식사 안해용"].map((n, i) => ({ restaurantId: 2, name: n, itemType: "main", sortOrder: i + 1 })),

  // 정우의 도시락 (3)
  ...["갈비 도시락", "간장불고기 도시락", "우삼겹 도시락", "삼겹 도시락", "제육 도시락", "스팸 도시락", "저녁식사 안해용"].map((n, i) => ({ restaurantId: 3, name: n, itemType: "main", sortOrder: i + 1 })),
  ...
  // 강남돈부리 (5)
  ...["가츠동", "볶음 김치가츠동 (김치돈까스덮밥)", "에비가츠동 (돈까스+왕새우)", "오야꼬동 (가라아게+계란덮밥)", "직화 매콤규동 (우삼겹 덮밥)", "매콤 돈카츠마요 오믈렛 덮밥", "매콤 치킨마요 오믈렛덮밥", "데미함박 오믈렛덮밥(경양식)", "저녁식사 안해용"].map((n, i) => ({ restaurantId: 5, name: n, itemType: "main", sortOrder: i + 1 })),

  // 극강은마 (6)
  ...["세세리동 (닭목살 덮밥)", "규동 (소고기덮밥)", "부타동 (돼지고기 덮밥)", "가라아게동 (튀김덮밥)", "저녁식사 안해용"].map((n, i) => ({ restaurantId: 6, name: n, itemType: "main", sortOrder: i + 1 })),

  // 오토김밥(역삼점) (7)
  ...["오토김밥(약간 매콤) + 닭강정", "스팸김밥 + 닭강정", "고추냉이김밥 + 닭강정", "오토닭강정 (청양고추 토핑)", "오토닭강정 (청양고추 토핑 빼기)", "저녁식사 안해용"].map((n, i) => ({ restaurantId: 7, name: n, itemType: "main", sortOrder: i + 1 })),

  // 이게볶음밥(강남점) (8)
  ...["삼겹김치 볶음밥", "리얼스팸김치 볶음밥", "철판닭갈비 볶음밥", "참치김치 볶음밥", "돼지불판 볶음밥", "햄야채볶음밥", "이게 제육덮밥 이지", "저녁식사 안해용"].map((n, i) => ({ restaurantId: 8, name: n, itemType: "main", sortOrder: i + 1 })),

  // 버텍스 (9)
  ...["미국식 닭고기 덮밥 S", "미국식 닭고기 덮밥 M", "미국식 닭고기 덮밥 L", "미국식 새우 닭고기 덮밥 S", "미국식 새우 닭고기 덮밥 M", "저녁식사 안해용"].map((n, i) => ({ restaurantId: 9, name: n, itemType: "main", sortOrder: i + 1 })),
  ...["데리야끼", "페퍼", "염염", "치폴레"].map((n, i) => ({ restaurantId: 9, name: n, itemType: "option", sortOrder: i + 1 })),

  // 타이반쩜 (10)
  ...["팟타이 (태국 대표 볶음면)", "미고랭", "타이칠리 (매움 1단계 / 신라면 UP)", "타이칠리 (매움 2단계 / 불닭 UP)", "팟씨유 (태국 간장 볶음면)", "칠리프라이드 (매운 볶음밥)", "카오팟 (파인애플 볶음밥)", "붓카케 냉우동정식 (냉우동+덴뿌라+유부초밥2p)", "텐붓카케 냉우동정식(냉우동+새우튀김+단호박+텐뿌라)", "에비덴뿌라 온우동정식(온우동+새우튀김+단호박+텐뿌라)", "토리텐 온우동정식(온우동+치킨가라아게+텐뿌라)", "돈등심카츠", "킹새우카츠", "모듬카츠", "규동", "가라아게동", "에비동", "저녁식사 안해용"].map((n, i) => ({ restaurantId: 10, name: n, itemType: "main", sortOrder: i + 1 })),

  // 미태리 파스타 (11)
  ...["미태리 치즈피자", "미태리 고르곤졸라 피자", "미태리 페퍼로니 피자", "베이컨 토마토 파스타", "베이컨 크림 파스타", "해물 짬뽕 파스타", "로제 크림 파스타", "매운 크림 파스타", "양송이 크림 파스타", "감바스 파스타", "토마토 치즈 리조또", "크림 치즈 리조또", "알리오올리오", "봉골레 파스타", "저녁식사 안해용"].map((n, i) => ({ restaurantId: 11, name: n, itemType: "main", sortOrder: i + 1 })),

  // 이삭토스트 (12)
  ...["감자스폐셜", "감자스폐셜 x2", "햄스페셜", "햄스페셜 x2", "베이컨 베스트", "그릴드 불갈비", "그릴드 불갈비 x2", "베이컨 포테이토 피자 x2", "딥치즈 베이컨 포테이토", "딥치즈 베이컨 포테이토 x2", "저녁식사 안해용"].map((n, i) => ({ restaurantId: 12, name: n, itemType: "main", sortOrder: i + 1 })),

  // 피자먹다 은마대치점 (13)
  ...["마르게리따 피자 1인 세트(감튀)", "달콤연유 피자 1인 세트(감튀)", "스위트고구마 피자 1인 세트(감튀)", "콤비네이션 피자 1인 세트(감튀)", "페퍼로니 피자 1인 세트(감튀)", "하와이언 피자 1인 세트(감튀)", "바질토마토 피자 1인 세트(감튀)", "마르게리따 피자&파스타 세트", "달콤연유 피자&파스타 세트", "스위트고구마 피자&파스타 세트", "저녁식사 안해용"].map((n, i) => ({ restaurantId: 13, name: n, itemType: "main", sortOrder: i + 1 })),

  // 파스타에 꼬치다 (14)
  ...["베이컨쪽파오일파스타 + 닭꼬치 1P", "베이컨크림파스타 + 닭꼬치 1P", "매콤크림파스타 + 닭꼬치 1P", "토마토파스타 + 닭꼬치 1P", "나폴리탄파스타 + 닭꼬치 1P", "매콤크림리조또 + 닭꼬치 1P", "저녁식사 안해용"].map((n, i) => ({ restaurantId: 14, name: n, itemType: "main", sortOrder: i + 1 })),

  // 샐러디 (15)
  ...["로스트 닭다리살 샐러드", "우삼겹 메밀면 누들볼", "노릇두부 단호박 샐러드", "탄단지 샐러드", "그라브락스 연어 샐러드", "칠리베이컨 포케볼", "바베큐닭다리살 포케볼", "로스트삼겹 포케볼", "저녁식사 안해용"].map((n, i) => ({ restaurantId: 15, name: n, itemType: "main", sortOrder: i + 1 })),
  ...
  // 포케올데이 (16)
  ...["곡물밥 포케", "메밀면 포케", "야채만 포케", "저녁식사 안해용"].map((n, i) => ({ restaurantId: 16, name: n, itemType: "main", sortOrder: i + 1 })),
  ...
  // 프로티너 (17)
  ...["닭가슴살 샐러드박스 S", "닭가슴살 샐러드박스 M", "돼지안심 샐러드박스 S", "돼지안심 샐러드박스 M", "폴드포크 샐러드박스 S", "폴드포크 샐러드박스 M", "소부채살 샐러드박스 S", "연어스테이크 샐러드박스 S", "저녁식사 안해용"].map((n, i) => ({ restaurantId: 17, name: n, itemType: "main", sortOrder: i + 1 })),
  ...["토마토살사", "레몬", "치미추리", "스파이시오리엔탈", "선택안함"].map((n, i) => ({ restaurantId: 17, name: n, itemType: "side", sortOrder: i + 1 })),

  // 맘스터치 (18)
  ...["싸이버거 세트", "싸이버거", "불싸이버거 세트", "불싸이버거", "싸이오리지널버거 세트", "싸이오리지널버거", "싸이클래식버거 세트", "싸이클래식버거", "모짜렐라 인더 버거 베이컨 세트", "모짜렐라 인더 버거 베이컨", "리아 새우 세트", "리아 새우", "한우 불고기버거 세트", "한우 불고기버거", "저녁식사 안해용"].map((n, i) => ({ restaurantId: 18, name: n, itemType: "main", sortOrder: i + 1 })),
  ...
  // 롯데리아 (19)
  ...["에그불고기버거 세트", "에그불고기버거", "리아 불고기버거 세트", "리아 불고기버거", "새우버거 세트", "새우버거", "모짜렐라인더버거 세트", "모짜렐라인더버거", "저녁식사 안해용"].map((n, i) => ({ restaurantId: 19, name: n, itemType: "main", sortOrder: i + 1 })),
